...
//         if (! _writing)
   ...
   //-----------------------------
   {
      if (_filename != file)
      {
         ...
         if (_output.is_open())
            _output.close();
         _output.open(file, std::ios::out | std::ios::binary | std::ios::ate);
         if (! _output.is_open())
            return false;
         _writecount = 0;
   ...
   //------------------------
   {
      if ( (writeon) && ( (! _output.is_open()) || (_filename.empty()) ) )
         return false;
      if ( (! _writing) && (writeon) )
      {
         std::unique_lock<std::mutex> lock(_writing_mutex);
      ...
      while (_running)
      {
         if (! _writing)
         {
            ...
            _writing_cond.wait_for(lock, std::chrono::milliseconds(200),
                                   [this] { return ( (this->_writing) || (! this->_running) ); });
//            if ( (_stopping) || (! _running) ) break;
            std::this_thread::yield();
            continue;
         }
...
//          ofs.close();
         ...
         if ( (_writing) && (_output.is_open()))
         {
            uint64_t v = to_big_endian64<int64_t>(data->_timestamp);
            _output.write((const char *) &v, sizeof(uint64_t));
            int write_size;
            ...
            else
            {
               ...
               v = to_big_endian64<int>(write_size);
               _output.write((const char *) &v, sizeof(uint64_t));
               _output.write(write_data, write_size);
               if (_must_compress)
                  delete write_data;
            }
            _writecount++;
...
private:
   ...
   volatile bool _writing = false, _running = false, _buffering = false;
